...
const Todo2 = () => {
    ...
    const handleToggleDone=(id,value)=>{
        const newArray=todo.map((item)=>item.id===id?{...item,done:value}:item
        )
        setTodo(newArray)
    }
  return (
        ...
        <div>
            {
                todo.map((item)=>(
                    ...
                    <div key={item.id}>
                        <div>
                            <input type="checkbox" checked={item.done} onChange={(e)=>handleToggleDone(item.id,e.target.checked)}  />
